...
get_data <- function(seu, assay = NULL, fit.model = "gaussian") {
    if (is.null(assay)) 
        assay <- DefaultAssay(seu)
    if (fit.model == "poisson") {
        dat <- Matrix::t(seu[[assay]]@counts) #原来是dat <- Matrix::t(seu[[assay]]@counts)
    }
    else if (fit.model == "gaussian") {
        dat <- Matrix::t(seu[[assay]]@data)  #原来是dat <- Matrix::t(seu[[assay]]@data)
    }
    else {
